...
def readMeta(meta_path, ascii_nodata, showCBar):
    ...
    return Meta(title, label, colormap, minColor, cMap, colorlisttype,
                ...
                xLabel, yLabel,
                YaxisMappingFile, YaxisMappingRefColumn, YaxisMappingTarColumn, YaxisMappingTarColumnAsF,
                YaxisMappingFormat,
                XaxisMappingFile, XaxisMappingRefColumn, XaxisMappingTarColumn, XaxisMappingTarColumnAsF,
                XaxisMappingFormat,
...
def createSubPlot(image, out_path, png_dpi, projectionID, pdf=None):
    ...
    if meta[0].border:
        projection = ccrs.epsg(projectionID)
    fig, axs = plt.subplots(nrows=nplotRows, ncols=nplotCols, squeeze=False, sharex=True, sharey=True,
                            figsize=image.size, subplot_kw={'projection': projection})
    ...
    # image.adhspace = 0.0
    # image.adwspace = 0.0
    fig.subplots_adjust(top=image.adjTop, bottom=image.adjBottom, left=image.adLeft, right=image.adRight,
                        wspace=image.adwspace, hspace=image.adhspace)
    ...
    for idxRow in range(1, nplotRows + 1):
        for idxCol in range(1, nplotCols + 1):
            ...
            for idxMerg in range(len(asciiHeaders)):
                ...
                plotLayer(fig, ax, idxCol, nplotCols, asciiHeader, meta, subtitle, onlyOnce, projectionID,
            ...
            if (len(metaInsertLs[(idxRow, idxCol)]) > 0 and
                ...
                for idxMerg in range(len(asciiHeaders)):
                    ...
                    subtitle = ""
                    onlyOnce = (idxMerg == len(asciiHeaders) - 1)
                    plotLayer(fig, inset_ax, idxCol, nplotCols, asciiHeader, meta, subtitle, onlyOnce, projectionID,
                              fontsize=fontsize, axlabelpad=axlabelpad, axtickpad=axtickpad)
...
def plotLayer(fig, ax, idxCol, numCols, asciiHeader, meta, subtitle, onlyOnce, projectionID, customLegend=None,
    ...
    if meta.renderAs == "heatmap":
        ...
        if meta.minLoaded and meta.maxLoaded:
            img_plot = ax.imshow(ascii_data_array, cmap=colorM, extent=image_extent, interpolation='none',
                                 vmin=meta.minValue, vmax=meta.maxValue)
        elif meta.minLoaded:
            img_plot = ax.imshow(ascii_data_array, cmap=colorM, extent=image_extent, interpolation='none',
                                 vmax=meta.minValue)
        elif meta.maxLoaded:
        ...
        if meta.showbars:
            ...
            if meta.ticklist:
                # Place a colorbar next to the map
                cbar = fig.colorbar(img_plot, ticks=meta.ticklist, orientation='vertical', shrink=0.5, aspect=14,
                                    cax=axins)
            else:
                # Place a colorbar next to the map
    ...
    if meta.renderAs == "densitySpread":
        ...
        if len(meta.lineLabel) > 0:
            if meta.lineLabelLoc == "none":
                ...
                ax.legend(fontsize=6, handlelength=1, bbox_to_anchor=(meta.lineLabelAnchorX, meta.lineLabelAnchorY),
                          loc=meta.lineLabelLoc)
    ...
    if meta.renderAs == "violinOccurrenceSpread":
        ...
        def calculateDistribution(occIndex, numberOfBuckets, inBucket, numRows, numInRow, allIndex=False):
            ...
            occurenceArray = calculateOccurrence(ascii_data_array, occIndex, numberOfBuckets, inBucket, numRows,
                                                 numInRow, allIndex)
        ...
        vp = ax.violinplot(listOfVPlots, listOfVPlotIdx, widths=1.5, showmeans=True, showmedians=False,
                           showextrema=True)
    ...
    if meta.renderAs == "stackedArea":
        ...
        if meta.occurrenceIndex != None:
            for occIndex in meta.occurrenceIndex:
                distr = calculateOccurrence(ascii_data_array, occIndex, numberOfBucketsC, inBucketC, numRowsC,
                                            numInRowC)
                if len(distr) > 0:
    ...
    if meta.renderAs == "densityCurvePlot":
        ...
        occurenceArray = calculateOccurrence(ascii_data_array, -1, numberOfBucketsC, inBucketC, numRowsC, numInRowC,
                                             allIndex=True)
